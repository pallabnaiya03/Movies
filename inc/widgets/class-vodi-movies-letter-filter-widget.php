 ...
 */
class Vodi_Movies_Letter_Filter_Widget extends MasVideos_Widget {

    /**
     * Constructor.
     */
    public function __construct() {
        $this->widget_cssclass    = 'vodi vodi-widget_movies_letter_filter';
        $this->widget_description = esc_html__( 'Display a list of letters to filter movies.', 'vodi' );
        $this->widget_id          = 'vodi_movies_letter_filter';
        $this->widget_name        = esc_html__( 'Vodi Filter Movies by Letter', 'vodi' );
        $this->settings           = array(
            'title'         => array(
                'type'          => 'text',
                'std'           => esc_html__( 'Filter Movies by Letter', 'vodi' ),
     ...
     * @return int
     */
    protected function get_movies_count() {
        global $wpdb;

        $sql = "SELECT LEFT( `post_title`, 1 ) as post_title_start, count(*) AS 'count' FROM {$wpdb->posts} WHERE post_type = 'movie' AND post_status = 'publish' GROUP BY post_title_start;";
    ...
    public function widget( $args, $instance ) {
        ...
        $range_0_9 = range( '0', '9' );
        $range_a_z = range( 'A', 'Z' );
        if( apply_filters( 'vodi_movies_letter_filter_display_numbers_group', true ) ) {
        ...
        $all_letters_count_info = $this->get_movies_count();
        ...
        foreach ( $filters_range as $letter ) {
            ...
            $class       = in_array( $letter, $letter_filter ) ? 'vodi-layered-nav-movies-letter chosen' : 'vodi-layered-nav-movies-letter';
            $link        = apply_filters( 'vodi_movies_letter_filter_link', $link_filter ? add_query_arg( 'letter_filter', $link_filter ) : remove_query_arg( 'letter_filter' ) );
            $letter_html = $letter;
